...
class RRTconnect:
    ...
    def __init__(self,start,goal,rand_area,expand_dis=0.1,path_resolution=0.05, goal_sample_rate = 1, max_iter=5000):
        ...
        third = (((start[0] + goal[0])/2 ), (start[1] + goal[1])/2) 
    ...
    def algorithm(self, animation=True):
        ...
        # Goal threshold
        pygame.draw.circle(screen, (0,255,0), (mf*self.end.x, mf*10-(mf*self.end.y)), 25)
        pygame.draw.circle(screen, (0,0,255), (mf*self.start.x, mf*10-(mf*self.start.y)), 25)
        pygame.draw.circle(screen, (255, 255, 255), (mf*self.third.x, mf*10-(mf*self.third.y)), 15)
        ...
        if counter == 0:
            ...
            for i in range(self.max_iter):
                ...
                new_node_grav_S = self.steerGrav(nearest_node_S, self.end, self.expand_dis)                
                ...
                new_node_grav_E = self.steerGrav(nearest_node_E, self.start, self.expand_dis)                
    ...
    def steerGrav(self, fromN, toN, extend_length = float("inf")):
        ...
        epsilon = 1    # gravitaitonal coefficient
        ...
        extend_length = min(biasedStepSize, extend_length)
        n_expand = math.floor(extend_length / self.path_resolution) # round down
...
def main():
    print("====================================================================================================")
    rrt = RRTconnect(start = [0,0], goal = [10,10], rand_area=[0, 1000])
