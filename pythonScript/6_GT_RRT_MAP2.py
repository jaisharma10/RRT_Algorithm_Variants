    ...
    def algorithm(self, animation=True):
        ...
        if counter == 0:
            ...
            for i in range(self.max_iter):
                ...
                # third node side ("====================================================================================================================================================")
                rnd_node_Th = self.getRandom()      # get random node
                nearest_ind_Th = self.getNodeIndex(self.V3,rnd_node_Th)                   # get nearest index
                nearest_node_Th = self.V3[nearest_ind_Th]                           # get node associated with nearest index
                new_node_Th = self.steer(nearest_node_Th, rnd_node_Th, self.expand_dis)       # get new node

                # check if the edge enters obstacle space
                if self.obstacleCheck(new_node_Th):
                    self.V3.append(new_node_Th)   # record newNode if valid
                    pygame.draw.circle(screen, (162, 136, 191), (100*new_node_Th.x, 100*10 - 100*new_node_Th.y), 4)
                    pygame.draw.line(screen, (121, 68, 179), (100*nearest_node_Th.x, 100*10 - 100*nearest_node_Th.y), (100*new_node_Th.x, 100*10 - 100*new_node_Th.y),3)
                    pygame.display.update()
                ...
                if t2e_best < 0.05 and s2t_best < 0.05:
                    print("Goal Reached!!")
                    # path, mid = self.backTrack(s_node_best, s2t_node_best, t2e_node_best, e_node_best)
                    ...
                    # display all 4 paths
                    for point in path1:
                        # pygame.draw.circle(screen, (255, 255, 255), (100*mid.x, 100*10 - 100*mid.y), 15)                       
...
def main():
    print("====================================================================================================")
    rrt = RRTconnect(start = [10,0], goal = [0,10], rand_area=[0, 1000])
